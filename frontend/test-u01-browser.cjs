...
async function main(){
  ...
  try{
    ...
    const script="let total=0;\nfor(let i=0;i<3;i++){total+=i;}\nprint(total);"
    await popup.locator('.CodeMirror').evaluate((e,text)=>{e.CodeMirror.setValue(text);e.CodeMirror.focus()},script)
    await popup.keyboard.press('Control+Enter')
    await popup.waitForTimeout(100)
    assert(calls.some(x=>x.token==='chen-B'&&x.action?.action==='run_sql'&&x.action.data===script))
    await popup.locator('.CodeMirror').evaluate(e=>{e.CodeMirror.setSelection({line:2,ch:0},{line:2,ch:13});e.CodeMirror.focus()})
    await popup.keyboard.press('Control+Enter')
    await popup.waitForTimeout(100)
    assert(calls.some(x=>x.token==='chen-B'&&x.action?.action==='run_sql'&&x.action.data==='print(total);'))
